...
// THIS IS THE ONLY PLACE WHERE THE MODEL NAME IS DEFINED - NO GCN ANYWHERE
const TEXT_MODEL_NAME = "Text GNN";
...
const ThreatDashboard = () => {
  ...
  const handleStartTraining = async () => {
    ...
    try {
      ...
      alert(`${selectedModel === 'text_gnn' ? TEXT_MODEL_NAME : 'Image ViT'} training completed!`);
    } catch (err) {
  ...
  const handleScrapeOnion = async () => {
    ...
    try {
      ...
      if (response.data?.results?.length > 0) {
        setScrapedData(response.data.results[0]);
      }
    ...
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2, flexWrap: 'wrap' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Chip label={isTextTrained ? `${TEXT_MODEL_NAME}: ✅` : `${TEXT_MODEL_NAME}: ❌`} sx={{ bgcolor: isTextTrained ? '#00bcd4' : '#ff4081', color: '#fff' }} />
      ...
      <Alert severity={bothReady ? 'success' : 'warning'} sx={{ mb: 3 }}>
        {bothReady ? '✅ Both models trained! Ready to scrape.' : `⚠️ Train ${TEXT_MODEL_NAME} and Image ViT models first.`}
      </Alert>
          ...
          <Paper sx={{ p: 3, bgcolor: '#1e293b' }}>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              <Select value={selectedModel} onChange={(e) => { setSelectedModel(e.target.value); setEpochs(e.target.value === 'text_gnn' ? 50 : 10); }} sx={{ color: '#fff' }}>
                <MenuItem value="text_gnn">📝 {TEXT_MODEL_NAME} (50 epochs)</MenuItem>
            ...
            <Button fullWidth variant="contained" startIcon={<PlayArrowIcon />} onClick={handleStartTraining} disabled={training} sx={{ mt: 2, bgcolor: '#ff4081' }}>
              {training ? 'Training...' : `Train ${selectedModel === 'text_gnn' ? TEXT_MODEL_NAME : 'Image ViT'}`}
            </Button>
            ...
            <Grid container spacing={1} sx={{ mt: 2 }}>
              <Grid item xs={6}>
                <Card sx={{ bgcolor: '#0f172a' }}>
                  <CardContent>
                    <Typography variant="caption">{TEXT_MODEL_NAME}</Typography>
                    <Typography variant="h6" sx={{ color: isTextTrained ? '#00bcd4' : '#ff4081' }}>
                      {isTextTrained ? '✓ Trained' : '✗ Not Trained'}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={6}>
                <Card sx={{ bgcolor: '#0f172a' }}>
                  <CardContent>
                    ...
                    <Typography variant="h6" sx={{ color: isImageTrained ? '#00bcd4' : '#ff4081' }}>
                      {isImageTrained ? '✓ Trained' : '✗ Not Trained'}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
        ...
        <Grid item xs={12} md={7}>
          <Paper sx={{ p: 3, bgcolor: '#1e293b' }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
              <TextField fullWidth placeholder="Enter .onion URL" value={onionUrl} onChange={(e) => setOnionUrl(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleScrapeOnion()} sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#00bcd4' } } }} />
              <Button variant="contained" onClick={handleScrapeOnion} disabled={scraping || !onionUrl.trim() || !bothReady} sx={{ bgcolor: bothReady ? '#00bcd4' : '#ff4081' }}>
            ...
            {scrapedData && (
              <Card sx={{ bgcolor: '#0f172a', mb: 2, border: '1px solid #00bcd4' }}>
                <CardContent>
                  <Typography variant="subtitle2" sx={{ color: '#00bcd4' }}>🎯 Threat Analysis</Typography>
                  <Grid container spacing={2} sx={{ mt: 1 }}>
                    <Grid item xs={3}><Typography variant="caption">Prediction</Typography><Chip label={scrapedData.text_classification?.prediction?.toUpperCase()} sx={{ bgcolor: getThreatColor(scrapedData.threat_level), color: '#fff', mt: 1 }} /></Grid>
                    <Grid item xs={3}><Typography variant="caption">Confidence</Typography><Typography variant="h6">{(scrapedData.text_classification?.confidence * 100).toFixed(1)}%</Typography></Grid>
                    <Grid item xs={3}><Typography variant="caption">Risk</Typography><Typography variant="h6">{scrapedData.overall_risk?.toFixed(0)}%</Typography></Grid>
                    <Grid item xs={3}><Typography variant="caption">Level</Typography><Chip label={scrapedData.threat_level} sx={{ bgcolor: getThreatColor(scrapedData.threat_level), color: '#fff', mt: 1 }} /></Grid>
                  </Grid>
                </CardContent>
              </Card>
            )}
            ...
            <List sx={{ maxHeight: 280, overflow: 'auto', bgcolor: '#0f172a', borderRadius: 1 }}>
              {recentScrapes.slice(0, 6).map((scrape, idx) => (
                <ListItem key={idx} sx={{ borderBottom: '1px solid #333', cursor: 'pointer' }} onClick={() => setOnionUrl(scrape.url)}>
                  ...
                  <ListItemText primary={scrape.url} secondary={`Risk: ${scrape.overall_risk?.toFixed(0)}% | ${scrape.prediction?.toUpperCase()}`} primaryTypographyProps={{ sx: { color: '#00bcd4' } }} />
                  <IconButton size="small" onClick={(e) => { e.stopPropagation(); setOnionUrl(scrape.url); }}><PlayArrowIcon sx={{ color: '#00bcd4' }} /></IconButton>
                </ListItem>
              ))}
